...
(def ^:private safe-context
  "A strictly locked down SCI context with no I/O, state mutations, or infinite sequence generators."
  (sci/init {:classes {'java.lang.String java.lang.String
  ...
  (when (> (count code) max-script-size)
    (throw (ex-info "Script exceeds maximum allowed size"
                    {:max max-script-size
                     :actual (count code)})))
  (validate-ast! code)
  (sci/eval-string* safe-context code))
