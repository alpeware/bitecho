...
- [ ] **Circuit Discovery (Ping/Pong):** Implement `:ping-peer` and `:pong-peer` event handlers in the pure state machine. `:ping-peer` must route toward the destination via the highest-staked peers, appending each hop to a `path` vector. `:pong-peer` must traverse the exact `path` in reverse to return the locked circuit to the origin.
