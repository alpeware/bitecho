...
- [ ] **Simulator Scaffolding & Config:** Update `deps.edn` to include a new `:simulate` alias. This alias must add a `dev/` folder to the `:extra-paths`. Create `dev/bitecho/simulator/core.clj` to hold the simulator namespace. This guarantees the chaos harness is ignored by the standard CI `:test` runner.
