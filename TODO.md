...
- [x] **Gossip TTL & Cache Pruning:** Modify the Contagion/Murmur message caching layer to track insertion timestamps or epoch numbers. Implement a periodic sweep (triggered by the existing `:tick` event) to drop message IDs and payloads older than a defined TTL. This prevents `contagion-known-ids` and `messages` maps from unbounded memory growth.
- [x] **Lottery Ticket Expiration:** Update the lottery ticket payload spec and validation logic to enforce a strict expiration window (e.g., tickets older than 24 hours are invalid). Modify the ledger's `claimed-tickets` logic to periodically garbage-collect expired tickets. This closes the replay attack vector while ensuring the ledger state remains mathematically bounded.
- [x] **Decentralized Difficulty Estimator:** Refactor `bitecho.economy.difficulty/calculate-difficulty`. Remove the `network-size` parameter from the external API API (`send-direct-message`). Instead, the pure state machine must dynamically estimate network scale by summing the Active Network Stake (Echo balances) of all peers currently observed in the `basalt-view` or `contagion` caches.
- [x] **Stake-Weighted Ingress (Protocol Level):** Implement an ingress prioritization mechanism for application-level messages (e.g., `:route-directed-message`). The protocol must check the sender's public key against the local `ledger` state and prioritize messages from senders with higher Echo UTXO balances. Implement a strict 5% "unstaked trickle lane" to ensure new agents with zero balances can still slowly route messages to earn their first Echos.

## Phase 9: Circuit Discovery & Quorum Settlement
- [x] **Circuit Discovery (Ping/Pong):** Implement `:ping-peer` and `:pong-peer` event handlers in the pure state machine. `:ping-peer` must route toward the destination via the highest-staked peers, appending each hop to a `path` vector. `:pong-peer` must traverse the exact `path` in reverse to return the locked circuit to the origin.
- [x] **Transient Proof of Relay:** Update directed messaging to require a locked circuit. Implement the cryptographic signature chaining (`[A -> R1 -> B -> R1 -> A]`) to mathematically validate successful round-trip delivery before a lottery ticket can be claimed.
- [x] **Dual-Ticket & Emission Curve:** Implement the Activity-Based Logarithmic Decay formula for Treasury minting. Differentiate the pure logic between high-difficulty Mint Tickets (drawing from the Treasury) and low-difficulty Fee Tickets (backed by sender UTXOs).
- [x] **Quorum Settlement:** Add pure reducer logic for a winning Mint Ticket & Transient Proof of Relay to unlock the Treasury UTXO. The holder of the winning ticket uses it's stake weighted routing to send the ticket & proof to ensure it gets incorporated in the majority holders local ledger.

- [x] **Decouple Tickets from Global Epoch:** Update `bitecho.lottery.core/winning-ticket?` and `bitecho.economy.ledger/claim-ticket`. Remove the `current-epoch` parameter and TTL expiration logic. Ticket validity must rely strictly on signature verification, the difficulty target, and the `claimed-tickets` replay-protection hash set.
- [x] **Local Circuit Garbage Collection:** Update `bitecho.state-machine`. Introduce a `pending-circuits` map to track initiated `:ping-peer` requests, tagged with the node's *local* epoch. Update the `handle-tick` reducer to prune any pending circuits older than a local TTL (e.g., 50 ticks), ensuring routing timeouts rely strictly on local time conventions.
- [x] **Phase 9 Integration Test:** Re-enable and rewrite `test/bitecho/shell/integration_test.clj`. Construct a 3-node scenario (A1 -> Boot -> A2). Sequence the complete flow: A1 pings A2, A2 pongs back, A1 sends `:route-directed-message` using the explicit locked circuit, A2 sends the `:route-directed-ack`, and the Boot router successfully claims the Fee Ticket on the return trip. Use robust `async/alts!!` timeouts.

## Phase 10: The Chaos Mesh Simulator
- [x] **Simulator Scaffolding & Config:** Update `deps.edn` to include a new `:simulate` alias. This alias must add a `dev/` folder to the `:extra-paths`. Create `dev/bitecho/simulator/core.clj` to hold the simulator namespace. This guarantees the chaos harness is ignored by the standard CI `:test` runner.
- [x] **The Orchestrator:** Implement the network orchestrator in the simulator namespace. It must read a config (e.g., `{:bootstraps 5, :agents 100, :spammers 10}`). It must generate keys, boot the respective shells, and wire all their `net-out` channels to a central multiplexer that routes messages to the correct `network-in` channels. It must also spawn a global metronome `go-loop` pushing `:tick` events to all nodes.
- [x] **The Byzantine Spammer:** Implement a custom, lightweight shell for malicious actors. These nodes bypass the pure state machine entirely. They run a tight loop constructing `{:type :route-directed-message}` envelopes with invalid signatures, zero-stake senders, or already-claimed lottery tickets, and blast them at the honest network to test the ingress trickle filters and ledger validation.
- [ ] **Telemetry & Scenario Script:** Write a `-main` execution script. It should boot the network, command honest agents to initiate random Ping/Pong sequences and payloads, and start the spammers. Create a "Telemetry Sink" that multiplexes all node `:app-out` channels to calculate and print real-time metrics: Time-to-circuit-lock, total treasury emitted, and ratio of dropped Byzantine packets vs. successful honest deliveries.
